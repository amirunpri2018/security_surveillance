...
def server_upload(object_name, object_score):
    ...
    files = {'snapshot': open('/home/pi/Desktop/security_serveillance/raspberry_pi/object_detection/frame.jpg', 'rb')}
    try:
        response = requests.post(url, files=files, data=detected_objects, timeout=3)
        print('Data has been uploaded on Server!')           
    except:
        print('Failed to upload on Server!')


snapshot_path = "/home/pi/Desktop/security_serveillance/raspberry_pi/object_detection/frame.jpg"
...
while True:
    ...
    if ('person' in object_name):  # If person is detected
        ...
        try:  # upload video
            if os.path.exists('/home/pi/Desktop/security_serveillance/raspberry_pi/object_detection/video_clip.avi'):
                pass  # do nothing because video recording is going on
            else:
                r = requests.get('http://127.0.0.1:8080/snapshot.clip', timeout=2)
                # call the url to save the video which will create timeout error after 2 sec
                # but video will be recorded 
        except:
